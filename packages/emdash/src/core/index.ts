import { DatabaseClient, SQLiteAdapter } from '@arrowera/db';
...
export class EmDashCMS {
  ...
  async getCollection(collectionName: string) {
    const results = await this.db.query('SELECT * FROM content WHERE type = ?', [collectionName]);
    return {
      entries: results.rows.map((r: any) => ({
        id: r.id,
        type: r.type,
        ...JSON.parse(r.data),
